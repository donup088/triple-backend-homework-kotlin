...
class DataLoader(
    ...
    companion object {
        val PLACE_LIST = mutableListOf(
            "북촌 한옥마을", "낙산공원", "올림픽공원", "서울숲", "뚝섬한강공원",
    ...
    override fun run(vararg args: String?) {
        if (userRepository.findAll().isEmpty()) {
            ...
            for (i in 1..10) {
                users.add(User(nickname = "tester$i"))
            }
        ...
        if (placeRepository.findAll().isEmpty()) {
            val placeList =
                PLACE_LIST.stream().map { placeName -> Place(name = placeName) }.collect(Collectors.toList())
